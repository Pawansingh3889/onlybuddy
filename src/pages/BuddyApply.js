...
const ADMIN_EMAIL    = 'YOUR_EMAIL@gmail.com';        // your email
const ADMIN_WHATSAPP = '447XXXXXXXXX';                // your WhatsApp e.g. 447712345678
...
export default function BuddyApply() {
  ...
  if (submitted) return (
    <div style={{ maxWidth: 600, margin: '80px auto', padding: '0 24px', textAlign: 'center' }}>
      <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 24, padding: 48 }}>
        ...
        <h1 style={{ fontSize: 32, fontWeight: 900, color: theme.green, fontFamily: "'Syne', sans-serif", marginBottom: 12 }}>Application Submitted!</h1>
  ...
  const inputStyle = { width: '100%', background: theme.card2, border: `1.5px solid ${theme.border}`, borderRadius: 10, padding: '11px 14px', color: theme.text, fontSize: 14, fontFamily: "'Plus Jakarta Sans', sans-serif", outline: 'none' };
  ...
  return (
    <div style={{ background: theme.bg, minHeight: '100vh' }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${theme.primaryDark}, ${theme.primary})`, padding: '48px 24px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontWeight: 900, color: '#fff', fontFamily: "'Syne', sans-serif", marginBottom: 12 }}>Become a Buddy</h1>
        ...
        <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 20, padding: '32px 28px' }}>
          ...
          {step === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: 4 }}>Personal Information</h2>
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: 4 }}>Address & Work Area</h2>
              <div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {VEHICLE_OPTIONS.map(v => (
                    <button key={v} onClick={() => set('vehicleType', v)} style={{ padding: '10px 18px', borderRadius: 10, border: `1.5px solid ${form.vehicleType === v ? theme.primary : theme.border}`, background: form.vehicleType === v ? theme.primaryBg : theme.card2, color: form.vehicleType === v ? theme.primary : theme.text2, fontWeight: form.vehicleType === v ? 700 : 500, fontSize: 13, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                  {[['availableWeekdays', '📅 Weekdays'], ['availableWeekends', '🎉 Weekends'], ['availableEvenings', '🌙 Evenings']].map(([field, label]) => (
                    <button key={field} onClick={() => set(field, !form[field])} style={{ padding: '10px 18px', borderRadius: 10, border: `1.5px solid ${form[field] ? theme.primary : theme.border}`, background: form[field] ? theme.primaryBg : theme.card2, color: form[field] ? theme.primary : theme.text2, fontWeight: form[field] ? 700 : 500, fontSize: 13, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                      {label}
                    </button>
                  ))}
                </div>
          ...
          {step === 2 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: 4 }}>Experience & References</h2>
              <div>
              ...
              <div style={{ background: theme.bg2, border: `1px solid ${theme.border}`, borderRadius: 14, padding: '20px' }}>
                <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 14, fontFamily: "'Syne', sans-serif" }}>🆘 Emergency Contact</div>
          ...
          {step === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: 4 }}>Upload Documents</h2>
          ...
          {step === 4 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: 4 }}>Review & Submit</h2>

              {/* Summary */}
              <div style={{ background: theme.bg2, borderRadius: 14, padding: 20 }}>
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12, fontFamily: "'Syne', sans-serif" }}>Your Application Summary</div>
                {[
                  ['Name', form.fullName],
                  ['Phone', form.phone],
                  ['Email', form.email],
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 28, gap: 12 }}>
            {step > 0 ? (
              <button onClick={() => { setStep(s => s - 1); setError(''); }} style={{ padding: '12px 24px', borderRadius: 12, border: `1px solid ${theme.border}`, background: theme.card2, color: theme.text2, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                ← Back
              </button>
            ) : <div />}

            {step < STEPS.length - 1 ? (
              <button onClick={next} style={{ padding: '12px 28px', borderRadius: 12, border: 'none', background: `linear-gradient(135deg, ${theme.primary}, ${theme.primaryDark})`, color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif", boxShadow: `0 4px 16px ${theme.primary}44` }}>
                Next Step →
              </button>
            ) : (
              <button onClick={submit} disabled={submitting} style={{ padding: '14px 32px', borderRadius: 12, border: 'none', background: submitting ? theme.muted : `linear-gradient(135deg, ${theme.green}, #047857)`, color: '#fff', fontSize: 15, fontWeight: 800, cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif", boxShadow: submitting ? 'none' : `0 4px 16px ${theme.green}44` }}>
