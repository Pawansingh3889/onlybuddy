  ...
  return (
    <div style={{ background: theme.bg, minHeight: '100vh' }}>
      ...
      <div style={{ background: theme.card, borderBottom: `1px solid ${theme.border}`, padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 900, color: theme.primary, fontFamily: "'Syne', sans-serif" }}>OnlyBuddy Admin</div>
      ...
      <div style={{ background: theme.card, borderBottom: `1px solid ${theme.border}`, padding: '0 24px', display: 'flex', gap: 4, overflowX: 'auto' }}>
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '14px 20px', background: 'none', border: 'none', borderBottom: `2px solid ${tab === t ? theme.primary : 'transparent'}`, color: tab === t ? theme.primary : theme.muted, fontWeight: tab === t ? 700 : 500, fontSize: 14, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif", whiteSpace: 'nowrap', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {pending.length > 0 && (
              <div style={{ background: theme.card, border: `1px solid ${theme.red}44`, borderRadius: 16, padding: 20 }}>
                <div style={{ fontSize: 16, fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: 14, color: theme.red }}>🚨 {pending.length} Application{pending.length > 1 ? 's' : ''} Awaiting Review</div>
                {pending.slice(0, 3).map(a => (
                  <div key={a.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: `1px solid ${theme.border}` }}>
                    ...
                    <button onClick={() => { setSelectedApp(a); setTab('Applications'); }} style={{ padding: '8px 16px', borderRadius: 8, border: `1px solid ${theme.primary}44`, background: theme.primaryBg, color: theme.primary, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Review →</button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 16, padding: 20 }}>
              <div style={{ fontSize: 16, fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: 14 }}>📋 Recent Orders</div>
              {orders.length === 0 ? (
              ...
              <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={() => setSelectedApp(null)}>
                <div style={{ background: theme.card, borderRadius: 20, padding: 32, maxWidth: 580, width: '100%', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                    <div>
                      <div style={{ fontSize: 22, fontWeight: 900, fontFamily: "'Syne', sans-serif" }}>{selectedApp.fullName}</div>
                  ...
                  <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                    <button onClick={() => approveApplication(selectedApp)} style={{ flex: 1, padding: 14, borderRadius: 12, border: 'none', background: `linear-gradient(135deg, ${theme.green}, #047857)`, color: '#fff', fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>✅ Approve</button>
                    <button onClick={() => rejectApplication(selectedApp)} style={{ flex: 1, padding: 14, borderRadius: 12, border: `1px solid ${theme.red}`, background: theme.redBg, color: theme.red, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>❌ Reject</button>
                  </div>
                </div>
              </div>
            )}
              ...
              : buddies.map(b => {
                ...
                return (
                  <div key={b.id} style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 14, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, marginBottom: 10 }}>
                    ...
                    {b.tier !== 'top' && (
                      <button onClick={() => upgradeTier(b.id, b.tier === 'new' ? 'trusted' : 'top')} style={{ padding: '8px 14px', borderRadius: 8, border: `1px solid ${theme.green}44`, background: theme.greenBg, color: theme.green, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>⬆️ Upgrade Tier</button>
                    )}
                  </div>
                );
              })
