    ...
    <div style={{ background: theme.bg, overflowX: 'hidden', width: '100%' }}>
      ...
      <section style={{ background: heroGradient, padding: 'clamp(48px, 10vw, 100px) 20px', textAlign: 'center', position: 'relative', overflow: 'hidden', width: '100%' }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', position: 'relative', width: '100%' }}>
          ...
          <h1 style={{ fontSize: 'clamp(32px, 8vw, 68px)', fontWeight: 900, color: '#fff', fontFamily: "'Syne', sans-serif", lineHeight: 1.1, marginBottom: 20, wordBreak: 'break-word' }}>
          ...
          <div className="hero-btns" style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', padding: '0 8px' }}>
            <Link to="/book" className="hero-btn" style={{ textDecoration: 'none', background: '#fff', color: theme.primaryDark, padding: '15px 32px', borderRadius: 14, fontSize: 16, fontWeight: 800, transition: 'all 0.2s', display: 'inline-block', fontFamily: "'Plus Jakarta Sans', sans-serif", whiteSpace: 'nowrap' }}>
              Book a Buddy →
            </Link>
            <Link to="/apply" className="hero-btn" style={{ textDecoration: 'none', background: 'rgba(255,255,255,0.12)', color: '#fff', padding: '15px 32px', borderRadius: 14, fontSize: 16, fontWeight: 700, border: '2px solid rgba(255,255,255,0.35)', transition: 'all 0.2s', display: 'inline-block', backdropFilter: 'blur(8px)', fontFamily: "'Plus Jakarta Sans', sans-serif", whiteSpace: 'nowrap' }}>
              Earn as a Buddy
            </Link>
          </div>
          ...
          <div className="stats-row" style={{ display: 'flex', gap: 24, justifyContent: 'center', marginTop: 48, flexWrap: 'wrap', padding: '0 8px' }}>
            {[
              ...
              <div key={label} className="stat-item" style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 'clamp(20px, 5vw, 28px)', fontWeight: 900, color: '#fff', fontFamily: "'Syne', sans-serif" }}>{val}</div>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(48px, 8vw, 80px) 20px', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <h2 style={{ fontSize: 'clamp(24px, 5vw, 40px)', fontWeight: 900, color: theme.text, fontFamily: "'Syne', sans-serif" }}>Every Errand Covered</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14 }}>
          {ERRAND_TYPES.map(e => (
            <div key={e.label} className="errand-card" style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 18, padding: 'clamp(16px, 4vw, 28px)', borderTop: `4px solid ${e.color}`, transition: 'all 0.2s', cursor: 'pointer' }} onClick={() => window.location.href='/book'}>
              ...
              <div style={{ fontSize: 15, fontWeight: 800, color: theme.text, fontFamily: "'Syne', sans-serif", marginBottom: 6 }}>{e.label}</div>
      ...
      <section style={{ background: theme.card, borderTop: `1px solid ${theme.border}`, borderBottom: `1px solid ${theme.border}`, width: '100%' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(48px, 8vw, 80px) 20px' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: 'clamp(24px, 5vw, 40px)', fontWeight: 900, color: theme.text, fontFamily: "'Syne', sans-serif" }}>How It Works</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 28 }}>
            {STEPS.map(s => (
              <div key={s.step}>
                ...
                <h3 style={{ fontSize: 17, fontWeight: 800, color: theme.text, fontFamily: "'Syne', sans-serif", marginBottom: 8 }}>{s.title}</h3>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(48px, 8vw, 80px) 20px', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <h2 style={{ fontSize: 'clamp(24px, 5vw, 40px)', fontWeight: 900, color: theme.text, fontFamily: "'Syne', sans-serif" }}>Why You Can Trust Your Buddy</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 }}>
          {TRUST_BADGES.map(b => (
            <div key={b.label} className="trust-card" style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 16, padding: '20px 22px', display: 'flex', gap: 14, transition: 'all 0.2s' }}>
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 800, color: theme.text, fontFamily: "'Syne', sans-serif", marginBottom: 4 }}>{b.label}</div>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px clamp(48px, 8vw, 80px)', width: '100%' }}>
        <div className="cta-grid" style={{ background: ctaGradient, borderRadius: 24, padding: 'clamp(32px, 6vw, 60px) clamp(24px, 5vw, 48px)', display: 'grid', gridTemplateColumns: '1fr auto', gap: 28, alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontSize: 'clamp(20px, 4vw, 36px)', fontWeight: 900, color: '#fff', fontFamily: "'Syne', sans-serif", marginBottom: 10 }}>Earn £10–£18/hr as a Buddy</h2>
          ...
          <Link to="/apply" className="cta-btn hero-btn" style={{ textDecoration: 'none', background: '#fff', color: theme.primaryDark, padding: '16px 32px', borderRadius: 14, fontSize: 15, fontWeight: 800, whiteSpace: 'nowrap', fontFamily: "'Plus Jakarta Sans', sans-serif", display: 'block', textAlign: 'center', transition: 'all 0.2s' }}>
            Apply Now →
          </Link>
        </div>
      </section>
