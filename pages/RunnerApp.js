    ...
    <div style={{ background: theme.bg, minHeight: '100vh', width: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <div style={{ padding: '16px 16px 12px', background: theme.card, borderBottom: `1px solid ${theme.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: 22, fontWeight: 900, color: theme.primary, fontFamily: "'Outfit', sans-serif" }}>OnlyBuddy 🤝</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                {[['Today', '£47.50', '💷'], ['Tasks', `${completedJobs.length + 5}`, '✅'], ['Rating', '4.97 ⭐', '🏆']].map(([l, v, i]) => (
                  <Card key={l} style={{ textAlign: 'center', padding: 14 }}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 900, color: theme.primary, fontFamily: "'Outfit', sans-serif", marginTop: 4 }}>{v}</div>
                      ...
                      return (
                        <Card key={job.id} style={{ padding: 16, borderLeft: `4px solid ${et?.color}` }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                            ...
                            <div style={{ fontSize: 20, fontWeight: 900, color: theme.green, fontFamily: "'Outfit', sans-serif" }}>£{job.pay.toFixed(2)}</div>
                          </div>
          ...
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <div style={{ padding: '14px 16px', background: theme.card, borderBottom: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 16, fontWeight: 800, fontFamily: "'Outfit', sans-serif" }}>Active Job</div>
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {showChat && (
                <Card style={{ padding: 0, overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '10px 14px', borderTop: `1px solid ${theme.border}`, display: 'flex', gap: 8 }}>
                    <input value={chatMsg} onChange={e => setChatMsg(e.target.value)} placeholder="Message customer..." style={{ flex: 1, background: theme.card2, border: `1px solid ${theme.border}`, borderRadius: 10, padding: '9px 12px', color: theme.text, fontSize: 13, outline: 'none', fontFamily: "'Inter', sans-serif" }} onKeyDown={e => e.key === 'Enter' && sendMsg()} />
              ...
              {/* Steps */}
              <Card>
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, fontFamily: "'Outfit', sans-serif" }}>Your Steps</div>
                {jobSteps.map((step, i) => (
        ...
        {tab === 'earnings' && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <div style={{ padding: '16px 16px 12px', background: theme.card, borderBottom: `1px solid ${theme.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
              <div style={{ fontSize: 20, fontWeight: 800, fontFamily: "'Outfit', sans-serif" }}>My Earnings</div>
            </div>
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <Card style={{ background: `linear-gradient(135deg, ${theme.primary}, ${theme.primaryDark})`, padding: 24, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 40, fontWeight: 900, color: '#fff', fontFamily: "'Outfit', sans-serif", marginTop: 4 }}>£127.50</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[['This Month', '£412.00'], ['Total Earned', '£3,847.50'], ['Avg per task', '£6.26'], ['Tasks done', '614']].map(([l, v]) => (
                  <Card key={l} style={{ padding: 14 }}>
                    <div style={{ fontSize: 20, fontWeight: 900, color: theme.primary, fontFamily: "'Outfit', sans-serif" }}>{v}</div>
                    <div style={{ fontSize: 11, color: theme.muted, marginTop: 3 }}>{l}</div>
                  </Card>
                ))}
              </div>
              <Card>
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12, fontFamily: "'Outfit', sans-serif" }}>Recent Payouts</div>
        ...
        {tab === 'profile' && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <div style={{ padding: '16px 16px 12px', background: theme.card, borderBottom: `1px solid ${theme.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
              <div style={{ fontSize: 20, fontWeight: 800, fontFamily: "'Outfit', sans-serif" }}>Buddy Profile</div>
            </div>
            <div style={{ padding: 16 }}>
              <Card style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 20, marginBottom: 14 }}>
                <Avatar emoji="🧑‍🦱" size={60} online={online} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "'Outfit', sans-serif" }}>Callum H.</div>
      ...
      <nav style={{ position: 'sticky', bottom: 0, width: '100%', background: theme.navBg, borderTop: `1px solid ${theme.border}`, display: 'flex', backdropFilter: 'blur(12px)', zIndex: 200 }}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: '10px 4px 12px', background: 'none', border: 'none', color: tab === t.id ? theme.primary : theme.muted, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', position: 'relative', fontFamily: "'Inter', sans-serif" }}>
